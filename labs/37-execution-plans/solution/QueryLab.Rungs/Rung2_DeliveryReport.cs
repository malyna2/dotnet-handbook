...
// Two collections in one JOIN multiply: every line is repeated once per shipment of its order.
// AsSplitQuery sends one statement per collection instead, each returning only its own rows.
